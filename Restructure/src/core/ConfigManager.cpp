...
namespace
{
bool parseServiceUrl(const QString &label,
                     ...
                     QString *errorMessage)
{
    ...
    if (trimmed.isEmpty() || !url.isValid() || url.host().isEmpty()
        || (scheme != QLatin1String("http") && scheme != QLatin1String("https")))
    {
        if (errorMessage)
        {
            *errorMessage = QStringLiteral("%1必须是包含主机名的 http/https 地址：%2")
                                .arg(label, trimmed);
        }
        return false;
    }
    ...
    const bool isLoopback = host == QLatin1String("localhost")
...
bool ConfigManager::resolveServiceTarget(const QString &homeUrl,
                                         const QString &apiUrl,
                                         bool *useLocalService,
                                         QString *errorMessage)
{
    bool homeUsesLoopback = false;
    bool apiUsesLoopback = false;
    if (!parseServiceUrl(QStringLiteral("首页地址"), homeUrl, &homeUsesLoopback, errorMessage)
        || !parseServiceUrl(QStringLiteral("接口地址"), apiUrl, &apiUsesLoopback, errorMessage))
    {
        return false;
    }

    if (homeUsesLoopback != apiUsesLoopback)
    {
        if (errorMessage)
        {
            *errorMessage = QStringLiteral(
                "首页地址和接口地址必须同时使用本机地址，或同时使用远程地址，不能混合配置。");
        }
        return false;
    }
    ...
    if (useLocalService)
    {
        *useLocalService = homeUsesLoopback;
    }
    if (errorMessage)
    {
        errorMessage->clear();
...
void ConfigManager::loadDefaults()
{
    ...
    if (!m_settings->contains("ticket/progressiveStepReturnEnabled"))
    {
        setValue("ticket/progressiveStepReturnEnabled", false);
    }
    if (!m_settings->contains("auth/accountListUrl"))
    {
        setValue("auth/accountListUrl", "");
    }
    if (!m_settings->contains("auth/accountListEnabled"))
    {
#ifdef Q_OS_WIN
        // PC deployments use direct password login and must not poll the account-list API.
        setValue("auth/accountListEnabled", false);
#else
        // Preserve the existing board behavior unless it is explicitly disabled.
        setValue("auth/accountListEnabled", true);
#endif
    }
