    ...
    , m_httpTransactionSafetyTimer(new QTimer(this))
{
    ...
    connect(m_ticketIngress, &TicketIngressService::cancelDeferredRequestExpired,
            this, &KeyManageController::handleCancelDeferredRequestExpired);
    ...
    m_ticketIngress->setCancelHandler([this](const QString &taskId) {
        ...
        if (trackedRecord.valid) {
            ...
            if (trackedRecord.cancelState == QLatin1String("cancel-accepted")
                ...
                result.accepted = false;
                result.message = QStringLiteral("任务撤销正在处理中，请勿重复提交");
                return result;
            }
        }
        ...
        // 已有撤销意图：处理中状态不能提前返回成功，其他历史状态保持原有幂等处理。
        if (!ticket.cancelState.isEmpty()
                && ticket.cancelState != QLatin1String("none")) {
            if (ticket.cancelState == QLatin1String("cancel-accepted")
                    || ticket.cancelState == QLatin1String("cancel-pending")
                    || ticket.cancelState == QLatin1String("cancel-executing")) {
                result.accepted = false;
                result.message = QStringLiteral("任务撤销正在处理中，请勿重复提交");
                qInfo().noquote()
                        << QStringLiteral("[KeyManageController] cancel ingress duplicate-active taskId=%1 cancelState=%2")
...
bool KeyManageController::canStartManualReturn(const QString &taskId, QString *blockedReason) const
{
    ...
    if (progressiveStepReturnEnabled() && isProgressiveStepTicket(ticket)) {
        if (blockedReason) {
            *blockedReason = QStringLiteral("该多步骤票已启用逐步回传，将在钥匙就绪后自动处理");
        }
        return false;
    }
...
void KeyManageController::handleSessionEvent(const KeySessionEvent &event)
{
    ...
    case KeySessionEvent::TasksUpdated: {
        ...
        if (!httpTransactionControlledEvent && ready) {
            tryStartProgressiveStepReturnSweep(tasks);
        }
        if (!httpTransactionControlledEvent && ready) {
    ...
    case KeySessionEvent::TaskLogReady: {
        ...
        if (m_progressiveReturnActive) {
            handleProgressiveTaskLog(event);
            break;
        }
...
{
    if (!m_progressiveHttpTaskId.isEmpty() && taskId == m_progressiveHttpTaskId) {
        const int serialNumber = m_progressiveHttpSerialNumber;
        m_progressiveHttpTaskId.clear();
        m_progressiveHttpSerialNumber = -1;
        if (serialNumber >= 0 && serialNumber < m_progressiveTotalSteps) {
            m_progressiveUploadedSteps[taskId].insert(serialNumber);
        }
        emit statusMessage(QStringLiteral("任务 %1 第 %2/%3 步回传成功")
                               .arg(taskId)
                               .arg(serialNumber + 1)
                               .arg(m_progressiveTotalSteps));
        if (!m_progressiveReturnActive
                || m_activeReturnTaskId != taskId
                || !isCurrentKeySession(m_activeReturnSessionId)) {
            clearProgressiveStepReturnContext(true);
            return;
        }
        uploadNextProgressiveStep();
        return;
    }
...
void KeyManageController::handleReturnUploadFailed(const QString &taskId, const QString &reason)
{
    if (!m_progressiveHttpTaskId.isEmpty() && taskId == m_progressiveHttpTaskId) {
        ...
        m_ticketStore->updateReturnState(taskId,
                                         ...
                                         reason);
        emit statusMessage(QStringLiteral("任务 %1 单步回传失败，父票保留：%2")
                               .arg(taskId, reason));
        finishProgressiveStepReturn(isCurrentKeySession(m_activeReturnSessionId));
        return;
    }
...
void KeyManageController::failActiveReturnHandshake(const QString &reason)
{
    ...
    if (m_progressiveReturnActive) {
        if (!m_progressiveHttpTaskId.isEmpty()) {
            m_progressiveCandidateTaskIds.clear();
            m_progressiveSweepSessionId = 0;
            emit statusMessage(QStringLiteral("逐步回传会话已中断，等待当前 HTTP 请求收口：%1").arg(reason));
            return;
        }
        finishProgressiveStepReturn(isCurrentKeySession(m_activeReturnSessionId));
        return;
...
void KeyManageController::finalizePendingReturnDelete(const QList<KeyTaskDto> &tasks)
{
    ...
    const SystemTicketDto pendingDeleteTicket =
            m_ticketStore->ticketById(m_pendingDeletedSystemTicketId);
    const bool strictProgressiveDelete = progressiveStepReturnEnabled()
            && m_pendingDeleteOrigin == DeleteOrigin::AutoReturnCleanup
            && isProgressiveStepTicket(pendingDeleteTicket);
    if (m_pendingDeleteSessionId > 0 && !isCurrentKeySession(m_pendingDeleteSessionId)) {
        if (strictProgressiveDelete) {
            m_pendingDeleteSessionId = 0;
            m_ticketStore->updateReturnState(
                    m_pendingDeletedSystemTicketId,
                    QStringLiteral("return-delete-verifying"),
                    QStringLiteral("逐步回传父票删除验证被钥匙移除打断，等待再次发现同一任务后继续核对"));
            emit statusMessage(QStringLiteral("逐步回传父票删除验证已中断，本地记录保留：%1")
    ...
    if (sessionUnknown && !stillExists) {
        if (strictProgressiveDelete) {
            ...
            m_ticketStore->updateReturnState(
                    taskId,
                    QStringLiteral("return-delete-verifying"),
                    QStringLiteral("会话变化后无法证明当前空任务列表来自原钥匙，本地记录保留待再次核对"));
            emit statusMessage(QStringLiteral("未能确认逐步回传父票已从原钥匙删除，本地记录保留：%1").arg(taskId));
            return;
        }
        if (m_pendingDeleteOrigin == DeleteOrigin::AdminClearOrphan) {
        ...
        if (m_pendingDeleteOrigin == DeleteOrigin::ManualDelete) {
            ...
            if (m_pendingDeleteAllowsRetransfer) {
                ...
                if (ticket.valid && (ticket.returnState == QLatin1String("return-delete-verifying")
                    ...
                    if (strictProgressiveDelete) {
                        m_progressiveUploadedSteps.remove(m_pendingDeletedSystemTicketId);
                    }
                    m_ticketStore->removeTicket(m_pendingDeletedSystemTicketId);
...
void KeyManageController::handleCancelDeferredRequestExpired(const QString &taskId)
{
    const QString tid = taskId.trimmed();
    if (tid.isEmpty()) {
        return;
    }

    const SystemTicketDto ticket = m_ticketStore->ticketById(tid);
    if (ticket.valid) {
        m_ticketCancelCoordinator->recordFailed(
                tid,
                ticket,
                QStringLiteral("撤销确认超时，已取消本次撤销意图，等待用户重新发起终止"));
        m_ticketStore->updateCancelTracking(
                tid,
                QStringLiteral("none"),
                QString(),
                QString(),
                ...
                QDateTime(),
                false,
                QStringLiteral("撤销确认超时，已取消本次撤销意图，需重新点击终止"));
    } else {
        SystemTicketDto transientTicket;
        transientTicket.taskId = tid;
    ...
    if (m_pendingCancelSystemTicketId == tid) {
    ...
    qInfo().noquote()
            << QStringLiteral("[KeyManageController] cancel deferred request expired, cancel intent cleared taskId=%1")
                  .arg(tid);
    emit statusMessage(QStringLiteral("撤销确认超时，已取消本次撤销，请重新点击终止任务：%1").arg(tid));
}
...
void KeyManageController::tryProcessPendingCancelTasks(const QList<KeyTaskDto> &tasks)
{
    ...
    for (const QString &taskId : pendingTaskIds) {
        ...
        if (keyTaskRaw.isEmpty()) {
            ...
            m_ticketIngress->completeDeferredCancelResponse(taskId, true, QStringLiteral("撤销票成功"));
...
bool KeyManageController::progressiveStepReturnEnabled() const
{
    return ConfigManager::instance()
            ->value(QStringLiteral("ticket/progressiveStepReturnEnabled"), false)
            .toBool();
}

bool KeyManageController::isProgressiveStepTicket(const SystemTicketDto &ticket) const
{
    return ticket.valid && ticket.stepNum > 1;
}

void KeyManageController::tryStartProgressiveStepReturnSweep(const QList<KeyTaskDto> &tasks)
{
    if (!progressiveStepReturnEnabled()
            || m_progressiveReturnActive
            || !m_progressiveHttpTaskId.isEmpty()
            || !m_progressiveCandidateTaskIds.isEmpty()
            || !activeReturnChainTaskId().isEmpty()
            || !m_activeTransferTaskId.isEmpty()
            || !m_pendingCancelSystemTicketId.isEmpty()
            || m_ticketCancelCoordinator->hasPendingWork()
            || !m_ticketReturnClient->isConfigured()) {
        return;
    }

    const KeySessionSnapshot snapshot = m_session->snapshot();
    if (!isSessionReadySnapshot(snapshot)
            || snapshot.commandInFlight
            || snapshot.recoveryWindowActive) {
        return;
    }

    m_progressiveSweepSessionId = m_keySessionId;
    for (const KeyTaskDto &task : tasks) {
        if (task.status != 0x01 && task.status != 0x02) {
            continue;
        }

        const QString taskId = taskIdFromRaw(task.taskId);
        const SystemTicketDto ticket = m_ticketStore->ticketById(taskId);
        if (!isProgressiveStepTicket(ticket)
                || !isKeyPresenceFresh(ticket)
                || (!ticket.cancelState.isEmpty() && ticket.cancelState != QLatin1String("none"))
                || isReturnCleanupState(ticket.returnState)
                || ticket.returnState == QLatin1String("return-success")
                || ticket.returnState == QLatin1String("return-delete-success")) {
            continue;
        }
        if (!m_progressiveCandidateTaskIds.contains(task.taskId)) {
            m_progressiveCandidateTaskIds.append(task.taskId);
        }
    }

    if (m_progressiveCandidateTaskIds.isEmpty()) {
        m_progressiveSweepSessionId = 0;
        return;
    }
    startNextProgressiveStepReturn();
}

void KeyManageController::startNextProgressiveStepReturn()
{
    if (m_progressiveReturnActive || !m_progressiveHttpTaskId.isEmpty()) {
        return;
    }
    if (!isCurrentKeySession(m_progressiveSweepSessionId)
            || !m_pendingDeletedSystemTicketId.isEmpty()
            || !m_pendingCancelSystemTicketId.isEmpty()
            || !m_activeTransferTaskId.isEmpty()
            || m_ticketCancelCoordinator->hasPendingWork()) {
    ...
    const KeySessionSnapshot snapshot = m_session->snapshot();
    if (!isSessionReadySnapshot(snapshot) || snapshot.recoveryWindowActive) {
        clearProgressiveStepReturnContext(true);
        return;
    }
    if (snapshot.commandInFlight) {
        QTimer::singleShot(150, this, [this]() { startNextProgressiveStepReturn(); });
        return;
    }

    while (!m_progressiveCandidateTaskIds.isEmpty()) {
        const QByteArray taskIdRaw = m_progressiveCandidateTaskIds.takeFirst();
        const QString taskId = taskIdFromRaw(taskIdRaw);
        quint8 taskStatus = 0xFF;
        const QByteArray currentRaw = findKeyTaskIdRaw(taskId, &taskStatus);
        const SystemTicketDto ticket = m_ticketStore->ticketById(taskId);
        if (currentRaw != taskIdRaw
                || (taskStatus != 0x01 && taskStatus != 0x02)
                || !isProgressiveStepTicket(ticket)
                || !isKeyPresenceFresh(ticket)
                || (!ticket.cancelState.isEmpty() && ticket.cancelState != QLatin1String("none"))
                || isReturnCleanupState(ticket.returnState)) {
            continue;
        ...
        m_progressiveReturnActive = true;
        m_progressiveTotalSteps = ticket.stepNum;
        m_progressiveStationNo = 0;
        m_progressiveObservedCompletedSteps.clear();
        m_progressivePendingItems.clear();
        m_activeReturnTaskId = taskId;
        ...
        req.opId = nextOpId();
        m_session->execute(req);
        emit statusMessage(QStringLiteral("逐步回传：开始读取任务 %1 的累计步骤日志").arg(taskId));
        return;
    }
...
void KeyManageController::handleProgressiveTaskLog(const KeySessionEvent &event)
{
    const QString eventTaskId = event.data.value(QStringLiteral("taskId")).toString();
    const QByteArray eventTaskIdRaw = event.data.value(QStringLiteral("taskIdRaw")).toByteArray();
    const int taskAttr = event.data.value(QStringLiteral("taskAttr"), -1).toInt();
    const int steps = event.data.value(QStringLiteral("steps"), -1).toInt();
    if (!m_progressiveReturnActive
            || !isCurrentKeySession(m_activeReturnSessionId)
            || eventTaskId != m_activeReturnTaskId
            || eventTaskIdRaw != m_activeReturnTaskIdRaw
            || taskAttr != 0
            || steps != m_progressiveTotalSteps) {
        const QString taskId = m_activeReturnTaskId;
        const QString reason = QStringLiteral("逐步回传日志校验失败：taskId、attr 或总步骤数不匹配");
        if (!taskId.isEmpty()) {
            m_ticketStore->updateReturnState(taskId,
                                             QStringLiteral("return-interrupted-retryable"),
                                             reason);
        ...
        emit statusMessage(reason);
        finishProgressiveStepReturn(true);
        return;
    }

    m_progressiveStationNo = event.data.value(QStringLiteral("stationNo")).toInt();
    QMap<int, QJsonObject> successfulItems;
    const QVariantList rawItems = event.data.value(QStringLiteral("items")).toList();
    for (const QVariant &itemVar : rawItems) {
        const QVariantMap itemMap = itemVar.toMap();
        const int serialNumber = itemMap.value(QStringLiteral("serialNumber"), -1).toInt();
        const int opStatus = itemMap.value(QStringLiteral("opStatus"), -1).toInt();
        if (opStatus != 1 || serialNumber < 0 || serialNumber >= m_progressiveTotalSteps) {
            continue;
        }
        successfulItems.insert(serialNumber, QJsonObject::fromVariantMap(itemMap));
    }

    m_progressiveObservedCompletedSteps.clear();
    m_progressivePendingItems.clear();
    const QSet<int> uploadedSteps = m_progressiveUploadedSteps.value(m_activeReturnTaskId);
    for (auto it = successfulItems.cbegin(); it != successfulItems.cend(); ++it) {
        m_progressiveObservedCompletedSteps.insert(it.key());
        if (!uploadedSteps.contains(it.key())) {
            m_progressivePendingItems.append(it.value());
        }
    }

    qInfo().noquote()
            << QStringLiteral("[KeyManageController] progressive log taskId=%1 completed=%2/%3 pendingUpload=%4")
                  .arg(m_activeReturnTaskId)
                  .arg(m_progressiveObservedCompletedSteps.size())
                  .arg(m_progressiveTotalSteps)
                  .arg(m_progressivePendingItems.size());
    uploadNextProgressiveStep();
}

void KeyManageController::uploadNextProgressiveStep()
{
    if (!m_progressiveReturnActive || !m_progressiveHttpTaskId.isEmpty()) {
        return;
    }
    if (m_progressivePendingItems.isEmpty()) {
        if (canDeleteProgressiveStepTicket()) {
            beginProgressiveStepDelete();
            return;
        }

        const int uploaded = m_progressiveUploadedSteps.value(m_activeReturnTaskId).size();
        const QString taskId = m_activeReturnTaskId;
        m_ticketStore->updateReturnState(
                taskId,
                QStringLiteral("return-interrupted-retryable"),
                QStringLiteral("已逐步回传 %1/%2，等待剩余步骤").arg(uploaded).arg(m_progressiveTotalSteps));
        emit statusMessage(QStringLiteral("任务 %1 已回传 %2/%3 步，父票继续保留")
                               .arg(taskId)
                               .arg(uploaded)
                               .arg(m_progressiveTotalSteps));
        finishProgressiveStepReturn(true);
    ...
    const QJsonObject item = m_progressivePendingItems.takeFirst();
    const int serialNumber = item.value(QStringLiteral("serialNumber")).toInt(-1);
    if (serialNumber < 0 || serialNumber >= m_progressiveTotalSteps) {
        uploadNextProgressiveStep();
        return;
    }
    ...
    QJsonObject payload;
    payload.insert(QStringLiteral("stationNo"), m_progressiveStationNo);
    payload.insert(QStringLiteral("steps"), m_progressiveTotalSteps);
    payload.insert(QStringLiteral("taskId"), m_activeReturnTaskId);
    QJsonArray items;
    items.append(item);
    payload.insert(QStringLiteral("taskLogItems"), items);

    m_progressiveHttpTaskId = m_activeReturnTaskId;
    m_progressiveHttpSerialNumber = serialNumber;
    m_ticketStore->updateReturnState(m_activeReturnTaskId, QStringLiteral("return-uploading"));
    emit statusMessage(QStringLiteral("任务 %1 正在回传第 %2/%3 步")
                           .arg(m_activeReturnTaskId)
                           .arg(serialNumber + 1)
                           .arg(m_progressiveTotalSteps));
    m_ticketReturnClient->uploadReturnPayload(payload);
...
bool KeyManageController::canDeleteProgressiveStepTicket() const
{
    if (!m_progressiveReturnActive
            || m_progressiveTotalSteps <= 0
            || !isCurrentKeySession(m_activeReturnSessionId)
            || m_progressiveObservedCompletedSteps.size() != m_progressiveTotalSteps) {
        return false;
    }

    const QSet<int> uploadedSteps = m_progressiveUploadedSteps.value(m_activeReturnTaskId);
    for (int serialNumber = 0; serialNumber < m_progressiveTotalSteps; ++serialNumber) {
        if (!m_progressiveObservedCompletedSteps.contains(serialNumber)
                || !uploadedSteps.contains(serialNumber)) {
            return false;
        }
    }

    quint8 currentStatus = 0xFF;
    const QByteArray currentRaw = findKeyTaskIdRaw(m_activeReturnTaskId, &currentStatus);
    return currentRaw == m_activeReturnTaskIdRaw && currentStatus == 0x02;
}

void KeyManageController::beginProgressiveStepDelete()
{
    if (!canDeleteProgressiveStepTicket()) {
        return;
    }
    ...
    const QString taskId = m_activeReturnTaskId;
    const QByteArray taskIdRaw = m_activeReturnTaskIdRaw;
    markReturnDeletePending(taskId, taskIdRaw);
    ...
    m_session->execute(req);
    emit statusMessage(QStringLiteral("任务 %1 的全部 %2 步均已回传成功，开始删除父票")
                           .arg(taskId)
                           .arg(m_progressiveTotalSteps));
    clearProgressiveStepReturnContext(true);
}

void KeyManageController::finishProgressiveStepReturn(bool continueSweep)
{
    const int sweepSessionId = m_progressiveSweepSessionId;
    clearProgressiveStepReturnContext(!continueSweep);
    if (!continueSweep || !isCurrentKeySession(sweepSessionId)) {
        m_progressiveCandidateTaskIds.clear();
        m_progressiveSweepSessionId = 0;
        return;
    }

    QTimer::singleShot(0, this, [this, sweepSessionId]() {
        if (isCurrentKeySession(sweepSessionId)) {
            startNextProgressiveStepReturn();
        } else {
            clearProgressiveStepReturnContext(true);
        }
...
void KeyManageController::clearProgressiveStepReturnContext(bool clearCandidates)
{
    m_progressiveReturnActive = false;
    m_progressiveTotalSteps = 0;
    m_progressiveStationNo = 0;
    m_progressiveObservedCompletedSteps.clear();
    m_progressivePendingItems.clear();
...
void KeyManageController::tryStartTicketReturn(const QString &taskId, bool automatic)
{
    const SystemTicketDto ticket = m_ticketStore->ticketById(taskId);
    if (progressiveStepReturnEnabled() && isProgressiveStepTicket(ticket)) {
        tryStartProgressiveStepReturnSweep(m_lastKeyTasks);
        if (!automatic) {
            emit statusMessage(QStringLiteral("该多步骤票已启用逐步回传，已按当前钥匙日志尝试处理"));
        }
        return;
    }
...
QString KeyManageController::activeReturnChainTaskId() const
{
    ...
    if (!m_progressiveHttpTaskId.isEmpty()) {
        return m_progressiveHttpTaskId;
    }
    return QString();
}
