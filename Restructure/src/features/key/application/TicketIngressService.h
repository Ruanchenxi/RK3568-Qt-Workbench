...
public:
    ...
    // 撤销票确认完成后再回复 cancelTicket，避免 H5 提前拿到 success=true。
    bool completeDeferredCancelResponse(const QString &taskId, bool success, const QString &message);

signals:
    void httpServerLogAppended(const QString &text);
    void jsonReceived(const QByteArray &jsonBytes, const QString &savedPath);
    void cancelDeferredRequestExpired(const QString &taskId);
...
private:
    ...
    void expireDeferredCancelRequest(const QString &taskId);

    QTcpServer *m_server;
    QHash<QTcpSocket *, QByteArray> m_buffers;
    QHash<QString, DeferredRequest> m_deferredRequests;  // taskId → 挂起的 socket
    QHash<QString, DeferredRequest> m_deferredCancelRequests;  // cancelTicket 专用，和 transTicket 隔离
    QString m_lastError;
