...
void SystemPage::onSaveClicked()
{
    if (ui->tabBasic->isChecked())
    {
        ...
        QString targetError;
        if (!ConfigManager::resolveServiceTarget(ui->homeUrlEdit->text(),
                                                 ui->apiUrlEdit->text(),
                                                 &useLocalService,
                                                 &targetError))
        {
            QMessageBox::warning(this, QStringLiteral("地址配置错误"), targetError);
            return;
        }
#endif
        ...
        loadSettings();
#ifdef Q_OS_WIN
        const QString serviceTargetText = useLocalService
                                              ? QStringLiteral("本机服务（将使用程序目录下的 outage）")
                                              : QStringLiteral("远程服务器（不会启动本地 outage）");
#else
        const QString serviceTargetText = QStringLiteral("当前服务配置");
#endif
        QMessageBox::information(this, QStringLiteral("保存配置"),
                                 QStringLiteral("配置已保存。\n\n"
                                                "服务地址已识别为：%1；\n"
                                                "请重启程序，使新的服务地址和启动策略生效；\n"
                                                "站号会同时影响 DEL 协议地址以及 INIT/RFID 后端取数范围；\n"
                                                "本页表单会立即显示新串口配置，但当前运行中的串口连接保持不变；\n"
                                                "请在当前任务完成后重启程序，再统一加载新的配置。")
                                     .arg(serviceTargetText));
    }
    else
    {
...
void SystemPage::saveSettings()
{
    ...
    dto.homeUrl = ui->homeUrlEdit->text().trimmed();
    dto.apiUrl = ui->apiUrlEdit->text().trimmed();
