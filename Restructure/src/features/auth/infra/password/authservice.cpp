...
void applyThirdApiHeaders(QNetworkRequest *request)
{
    if (!request)
    {
        return;
    ...
    // The online outage gateway expects this marker on third-api calls.
...
void AuthService::sendLoginRequest(const QString &userName, const QString &encryptedPassword, const QString &tenantId)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, userName]()
        ...
        if (reply->error() != QNetworkReply::NoError) {
            ...
            qDebug() << "[AuthService] login network error:"
                     << reply->error()
                     << reply->errorString()
