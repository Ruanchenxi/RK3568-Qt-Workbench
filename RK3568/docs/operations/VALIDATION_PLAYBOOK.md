...
6. 错方法请求应返回：
   - `405`
7. 请求行非法 / `Content-Length` 非法 / JSON 无法入池时应返回：
   - `400`
8. 只有业务真正入池成功时，HTTP 才应返回：
   - `success=true`
9. 若请求被业务接受但按规则忽略（例如重复任务），HTTP 可返回 `success=true`，但 `msg` 必须体现真实业务语义，而不是统一 `ticket accepted`
5. 负向验证补充：
   - 错路径应返回 `404`
   - 错方法应返回 `405`
   - 非法请求行 / 缺失或非法 `Content-Length` / 非法 JSON / `taskId` 为空应返回 `400`
   - 只有业务真正入池成功时才返回 `success=true`
...
## 15. 2026-03-23 键盘验证补充

1. 登录页 / 系统设置页：
   - 允许先聚焦输入框再打开键盘
   - 也允许先打开键盘，再落到当前输入目标
2. 工作台页：
   - 必须先点击网页中的可编辑输入元素，再点击顶部 `虚拟键盘`
   - 若未先聚焦网页输入元素，应提示后保持键盘关闭
3. 钥匙管理页 / 服务日志页：
   - 当前不作为有效输入页，不应承诺“弹出后可直接输入”
4. 跨页切换验证：
