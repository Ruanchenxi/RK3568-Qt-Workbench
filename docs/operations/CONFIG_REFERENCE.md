...
5. `ui/keyboarddialog.ui` 属于历史 dialog 键盘方案：
   ...
   - 不属于当前正式键盘主线
   - 当前已移出工程主线构建清单
   - 当前仅保留磁盘文件，不再属于工程主线路径
   - 当前应视为删除候选，不应继续当作现行方案扩写
6. 旧 Qt Virtual Keyboard 过渡链当前口径是：
   - 默认关闭
   - 本轮暂不删除
   - 只有在完成入口映射、资源装配核对与验证基线后，才允许进入真正删除评估
7. 日常维护时，若当前任务不是“旧键盘 / 旧输入法链清理专项”，默认可忽略上述旧 provider 配置项与 `keyboarddialog.ui`。
