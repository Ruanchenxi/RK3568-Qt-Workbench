...
KeySerialClient::KeySerialClient(QObject *parent)
    : QObject(parent)
    ...
    , m_currentOpId(-1)
{
    connect(m_serial, &QSerialPort::readyRead, this, &KeySerialClient::onReadyRead);
    connect(m_serial, &QSerialPort::errorOccurred, this, &KeySerialClient::onSerialError);
...
int KeySerialClient::tryExtractOneFrame(QByteArray &buf, QByteArray &outFrame)
{
    while (true) {
        ...
        if (headerPos < 0) {
            if (buf.size() > 1) {
                ...
                emitLog(LogDir::WARN, 0, 0,
                        QString("RESYNC no-header dropBytes=%1 recvBufBefore=%2 recvBufAfter=1")
                            .arg(dropped)
                            .arg(recvBufBefore),
                        buf.left(dropped), true, true);
        ...
        if (headerPos > 0) {
            ...
            emitLog(LogDir::WARN, 0, 0,
                    QString("RESYNC drop %1 bytes failReason=headerShift droppedByteCount=%1 nextHeaderIndex=0 recvBufBefore=%2")
                        .arg(headerPos)
                        .arg(recvBufBefore),
                    buf.left(headerPos), true, true);
        ...
        if (lenField < 1 || lenField > (1 + MAX_DATA_LEN)) {
            ...
            emitLog(LogDir::ERROR, 0, lenField,
                    QString("RESYNC failReason=LEN_INVALID droppedByteCount=1 nextHeaderIndex=%1 LenField=%2 expectedTotal=%3 lenInvalidCount=%4 recvBufNow=%5")
                        .arg(nextHeaderIndex)
                        .arg(lenField)
                        .arg(expectedTotal)
                        .arg(m_lenInvalidCount)
                        .arg(buf.size()),
                    {}, false, true);
            continue;
        }
        ...
        if (calcedCrc != recvCrc) {
            ...
            const int nextHeaderIndex = buf.indexOf(QByteArray::fromHex("7E6C"));
            emitLog(LogDir::ERROR, 0, lenField,
                    QString("RESYNC failReason=CRC_FAIL droppedByteCount=1 nextHeaderIndex=%1 LenField=%2 expectedTotal=%3 crcFailCount=%4 calc=0x%5 recv=0x%6")
                        .arg(nextHeaderIndex)
                        .arg(lenField)
                        .arg(expectedTotal)
                        .arg(m_crcFailCount)
                        .arg(calcedCrc, 4, 16, QChar('0'))
                        .arg(recvCrc, 4, 16, QChar('0')),
                    buf.left(qMin(buf.size(), expectedTotal)), false, true);
            continue;
        }
 ...
 */
bool KeySerialClient::isHardSerialError(QSerialPort::SerialPortError error) const
{
    return error == QSerialPort::ResourceError ||
...
void KeySerialClient::tryReopenVerifiedPort(const QString &reason)
{
    ...
    emitLog(LogDir::WARN, 0, 0,
            QString("串口硬错误，尝试重连同一端口 %1").arg(targetPort));
 ...
 */
void KeySerialClient::onSerialError(QSerialPort::SerialPortError error)
{
    ...
    emitLog(LogDir::ERROR, 0, 0,
            QString("SerialError(%1): %2").arg(static_cast<int>(error)).arg(err));

    if (!isHardSerialError(error)) {
        return;
    }
...
void KeySerialClient::onRetryTimeout()
{
    ...
    switch (m_expectedAckCmd) {
    case CMD_ACK:    retrySummary += " for SET_COM/DEL"; break;
    case CMD_Q_TASK: retrySummary += " for Q_TASK"; break;
    default:
...
{
    LogItem item;
    item.ts       = QDateTime::currentDateTime();
    item.port     = m_serial->portName();
    item.dir      = dir;
    item.opId     = (dir == LogDir::EVENT) ? -1 : m_currentOpId;
    item.cmd      = cmd;
    item.lenField = lenField;
