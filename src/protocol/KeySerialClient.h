 ...
 *
 * 依赖关系：
 *   - QSerialPort / QTimer / QElapsedTimer（Qt 串口和定时器）
 *   - KeyProtocolDefs.h（协议常量）
 *   - KeyCrc16.h（CRC16 计算，通过 calcCrc 间接调用）
 *   - LogItem.h（结构化日志条目）
 *
 * 被依赖：
 *   - KeyManagePage（创建实例、调用公共接口、连接信号槽）
 */
...
public:
    explicit KeySerialClient(QObject *parent = nullptr);
    ~KeySerialClient();
...
private slots:
    ...
    void onSerialError(QSerialPort::SerialPortError error);  ///< 串口硬错误处理
...
private:
    ...
    bool isHardSerialError(QSerialPort::SerialPortError error) const; ///< 判断是否为硬错误
    ...
    // 串口与接收缓冲
    QSerialPort   *m_serial;       ///< 串口实例（生命周期随 KeySerialClient）
